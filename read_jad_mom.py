...
def plot_jp_jh_data(jp,jh,j,sig_max,win,orbit,maxR):

    fig, ax = plt.subplots(5,1,sharex=True)
    fig.set_size_inches((12,8))
    wh = (jp.data_df.n_sig < sig_max) & (jp.data_df.n > 0) & (jp.bc_id == 1) & (jp.R < maxR)
    ...
    wh = (jh.data_df.n_sig < sig_max) & (jh.data_df.n > 0) & (jh.bc_id == 1) & (jh.R < maxR)
    ...
    wh = (jp.data_df.Temp > 0) & (jp.data_df.Temp_sig < sig_max*10) & (jp.bc_id == 1) & (jp.R < maxR)
    ax[4].plot(jp.data_df.Temp[wh].rolling(win).mean(),'.',markersize=1.0)
    wh = (jh.data_df.Temp > 0) & (jh.data_df.Temp_sig < sig_max*10) & (jh.bc_id == 1) & (jh.R < maxR)
...
def write_snippet(b,timestart,timeend):
    filename='./mag_df.csv'
    wh = (b.t > timestart) & (b.t < timeend)
    d = {'Br': b.Br[wh], 'Btheta': b.Btheta[wh], 'Bphi': b.Bphi[wh], 'R': b.R[wh], 'zcent': b.z_cent[wh]}
    df = pd.DataFrame(data = d)
    df.index = b.t[wh]
    df.to_csv(filename)
    
def get_Walen(jp,b,winsz):
    ...
    ax[0].plot(bt.index[wh],abs(bpar[wh]),'.',markersize=1.0)
    ax[0].plot(bt.index[wh],bperp[wh],'.',markersize=1.0)
    ...
    maxsig = 10
    wh = (n_sig1 < maxsig) & (n_sig2 < maxsig) & (vr_sig1 < maxsig) & (vr_sig2 < maxsig) & (vtheta_sig1 < maxsig) & (vtheta_sig2 < maxsig) & (vphi_sig1 < maxsig) & (vphi_sig2 < maxsig) 
...
def pres_bal(jh,b):
    ...
    wh = (abs(jh.R) > 35) & (abs(jh.R) < 50) & (jh.data_df.n_sig < 1000) & (jh.data_df.index < tpj[0])
...
def get_mag_flux_transport(jp,b):
    ...
    bt = bt.interpolate().reindex(jp.data_df.index)
    wh = (abs(jp.R) > 10) & (abs(jp.R) < 30) & (abs(jp.z_cent) < 2) & (jp.data_df.vr_sig < 1000)
...
def get_mdot(jp,jh,j,orbit):
    ...
    tpj = jp.t[jp.R == jp.R.min()]
    wh = (jp.data_df.n_sig < 1000) & (jp.z_cent < 1) & (jp.z_cent >-1) & (jp.bc_id == 1) & (jp.R > 10) & (jp.t < tpj[0])
    #plt.figure()
    #plt.plot(jp.data_df.n[wh],'.',markersize=1.0)
    #plt.yscale('log')
    wh = (jh.data_df.n_sig < 1000) & (jh.data_df.vr_sig < 1000) & (jh.z_cent < 1) & (jh.z_cent >-1) & (jh.bc_id == 1) & (jh.R > 10) & (jh.t < tpj[0]) & (jh.R < 60)
    #plt.figure()
...
def get_vphi_R(jp,orbit):
    tpj = jp.t[jp.R == jp.R.min()]
    wh = (jp.data_df.vphi_sig < 1000) & (np.abs(jp.z_cent) < 4) & (jp.bc_id == 1) & (jp.R > 10) #& (jp.t < tpj[0])
    #plt.figure()
...
def get_vphi_z(jp,orbit):
    tpj = jp.t[jp.R == jp.R.min()]
    wh = (jp.data_df.vphi_sig < 1000) & (jp.bc_id == 1) & (jp.R > 10) & (jp.R < 50)#& (jp.t < tpj[0])
    #plt.figure()
    #plt.plot(jp.R[wh],jp.data_df.vphi[wh],'.')
    #plt.show()
...
plt.xlabel('Radial Distance (RJ)')
plt.title('Orbits 5-12: z_cent < 4')
...
write_snippet(b,timestart,timeend)
